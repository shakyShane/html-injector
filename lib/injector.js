...
function getInjector(sockets, logger) {
    ...
    return function (window, diffs) {
        ...
        diffs.forEach(function (item) {
            ...
            item.diffType = item.diffType || "node";
            ...
            if (element) {
                ...
                logger.debug("EVENT:     {yellow:%s}", emitCount);
                logger.debug("DIFF TYPE: {yellow:%s", item.diffType);
                logger.debug("InnerHTML: {yellow:%s", element.innerHTML);
